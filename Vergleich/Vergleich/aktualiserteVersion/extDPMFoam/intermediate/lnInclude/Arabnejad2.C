...
#include "Arabnejad2.H"

// * * * * * * * * * * * * * Protectd Member Functions * * * * * * * * * * * //

template<class CloudType>
Foam::label Foam::Arabnejad2<CloudType>::applyToPatch
(
...
template<class CloudType>
void Foam::Arabnejad2<CloudType>::write()
{
    if (Arabnejad2Ptr_.valid())
    {
        Arabnejad2Ptr_->write();
    }
    else
    {
        FatalErrorInFunction
            << "Arabnejad2Ptr not valid" << abort(FatalError);
    }
}
...
template<class CloudType>
Foam::Arabnejad2<CloudType>::Arabnejad2
(
...
template<class CloudType>
void Foam::Arabnejad2<CloudType>::preEvolve()
{
    if (Arabnejad2Ptr_.valid())
    {
        Arabnejad2Ptr_->primitiveFieldRef() = 0.0;
    }
    else
    {
        const fvMesh& mesh = this->owner().mesh();

        Arabnejad2Ptr_.reset
        (
            new volScalarField
            (
                IOobject
...
template<class CloudType>
void Foam::Arabnejad2<CloudType>::postPatch
(
    const parcelType& p,
    const polyPatch& pp,
    bool&
)
{
    ...
    if (localPatchi != -1)
    {
 	...
 	const scalar M = p.nParticle()*p.mass();
        ...
        scalar& A2 = Arabnejad2Ptr_->boundaryFieldRef()[patchi][patchFacei];

	if (alpha < atan(K_) && magU*sin(alpha) > Utsh_)
        {
		A2 += Fs_ * d_ * (((0.5*M*pow((magU*sin(alpha) - Utsh_),2))/(epsilonA_)) + (C_*((M*pow(magU,2)*sin(alpha)*(2*K_*cos(alpha)-sin(alpha)))/(2*pow(K_,2)*P_))))/M;//modifiziert
	}
        else if (alpha >= atan(K_) && magU*sin(alpha) > Utsh_)
        { 
		A2 += Fs_ * d_ * (((0.5*M*pow((magU*sin(alpha) - Utsh_),2))/(epsilonA_)) + (C_*((M * pow(magU,2)*pow(cos(alpha),2))/(2*P_))))/M; //modifiziert
	}
	else
	{
		A2 += 0;
